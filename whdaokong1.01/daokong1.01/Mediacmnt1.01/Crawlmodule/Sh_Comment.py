...
class Sh_Comment:
    ...
    def hot_comment(self):
        ...
        for hot in hots:
            ...
            self.hot_list.append({'Comment_id':Comment_id,'Name': Name,'Content':Content,'Agree':str(Agree),'Time':Time,'Source_id':self.source_id,'Topic_id':Topic_id})
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(pageurl) as pagehtml:
                ...
                for cmnt in cmnts:
                    ...
                    # Source_id = html['jsonObject']['source_id']
                    Topic_id = html['jsonObject']['topic_id']
                    self.cmnt_list.append({'Comment_id': Comment_id, 'Name': Name, 'Content': Content, 'Agree': str(Agree), 'Time': Time,'Source_id':self.source_id,'Topic_id':Topic_id})
    def main(self):
